...
public class ProviderResource {
    ...
    public ResponseEntity<Provider> createProvider(@RequestBody Provider provider) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/providers/" + provider.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, provider.getId().toString()))
            .body(provider);
    }
        ...
        Optional<Provider> result = providerRepository
            ...
            .map(existingProvider -> {
                if (provider.getProvider() != null) {
                    existingProvider.setProvider(provider.getProvider());
                }
                if (provider.getAbn() != null) {
                    existingProvider.setAbn(provider.getAbn());
                }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<Provider>> getAllProviders(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
